...
void init_solvers(py::module_& m)
{
    ...
    py::class_<TortuosityHypre>(m, "TortuosityHypre",
        ...
        .def("value", [](TortuosityHypre& self, bool refresh) {
                ...
                if (std::isnan(val)) {
                    ...
                    throw std::runtime_error(
                        "TortuosityHypre.value() failed: " + reason);
                }
                return val;
             },
        ...
        .def("value_raw", [](TortuosityHypre& self, bool refresh) {
                return self.value(refresh);
             },
             py::arg("refresh") = false,
             "Return tortuosity (NaN on failure, no exception).")

        .def("check_matrix_properties", &TortuosityHypre::checkMatrixProperties,
             "Verify HYPRE matrix / RHS properties (for debugging).")

        // Read-only diagnostics
        .def_property_readonly("solver_converged",
             &TortuosityHypre::getSolverConverged)
        .def_property_readonly("residual_norm",
             &TortuosityHypre::getFinalRelativeResidualNorm)
        .def_property_readonly("iterations",
             &TortuosityHypre::getSolverIterations)
        .def_property_readonly("flux_in",
             &TortuosityHypre::getFluxIn)
        .def_property_readonly("flux_out",
             &TortuosityHypre::getFluxOut)
        .def_property_readonly("active_volume_fraction",
             &TortuosityHypre::getActiveVolumeFraction)
        .def_property_readonly("plane_fluxes",
             &TortuosityHypre::getPlaneFluxes)
        .def_property_readonly("plane_flux_max_deviation",
             &TortuosityHypre::getPlaneFluxMaxDeviation)
        .def_property_readonly("is_multi_phase",
             &TortuosityHypre::isMultiPhase);

    // =========================================================================
    // TortuosityDirect
    // =========================================================================
    py::class_<TortuosityDirect>(m, "TortuosityDirect",
        ...
        .def(py::init<const amrex::Geometry&,
             ...
             py::arg("eps"),
             py::arg("n_steps"),
             py::arg("plot_interval"),
             py::arg("plot_basename"),
             py::arg("vlo"),
             py::arg("vhi"),
             py::keep_alive<1, 2>(),   // geom
             py::keep_alive<1, 3>(),   // ba
        ...
        .def("value", [](TortuosityDirect& self, bool refresh) {
                amrex::Real val = self.value(refresh);
                if (std::isnan(val)) {
                    throw std::runtime_error(
        ...
        .def("value_raw", [](TortuosityDirect& self, bool refresh) {
                return self.value(refresh);
             },
             py::arg("refresh") = false)

        .def_property_readonly("num_iterations",
             &TortuosityDirect::getNumIterations)
        .def_property_readonly("final_residual",
             &TortuosityDirect::getFinalResidual);

    // =========================================================================
    // EffectiveDiffusivityHypre
    // =========================================================================
    py::class_<EffectiveDiffusivityHypre>(m, "EffectiveDiffusivityHypre",
        ...
        .def(py::init<const amrex::Geometry&,
                       ...
                       int,
                       OpenImpala::Direction,
                       EffectiveDiffusivityHypre::SolverType,
                       const std::string&,
                       int,
                       bool>(),
             py::arg("geom"),
        ...
        .def("get_chi_solution", &EffectiveDiffusivityHypre::getChiSolution,
             py::arg("chi_field"),
             "Copy the solved corrector field into *chi_field*.")

        .def_property_readonly("solver_converged",
             &EffectiveDiffusivityHypre::getSolverConverged)
        .def_property_readonly("residual_norm",
             &EffectiveDiffusivityHypre::getFinalRelativeResidualNorm)
        .def_property_readonly("iterations",
             &EffectiveDiffusivityHypre::getSolverIterations);
}
