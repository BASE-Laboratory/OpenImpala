...
void init_io(py::module_& m)
{
    ...
    // TiffReader
    // =========================================================================
    py::class_<TiffReader>(m, "TiffReader",
        ...
        .def("read_file", &TiffReader::readFile,
             py::arg("filename"),
             "Read metadata from a single TIFF (clears previous state).")

        .def("read_file_sequence", &TiffReader::readFileSequence,
             py::arg("base_pattern"),
             py::arg("num_files"),
             py::arg("start_index") = 0,
             py::arg("digits") = 1,
        ...
        .def("threshold",
             py::overload_cast<double, amrex::iMultiFab&>(
                 &TiffReader::threshold, py::const_),
             py::arg("raw_threshold"),
             py::arg("mf"),
             "Fill *mf* with 1 where pixel > threshold, else 0.")

        // Four-arg threshold (custom output values)
        .def("threshold",
             py::overload_cast<double, int, int, amrex::iMultiFab&>(
                 &TiffReader::threshold, py::const_),
             py::arg("raw_threshold"),
             py::arg("value_if_true"),
             py::arg("value_if_false"),
             py::arg("mf"),
             "Fill *mf* with custom values based on threshold comparison.")
        ...
        .def("__repr__", [](const TiffReader& r) {
            if (!r.isRead()) return std::string("<TiffReader (no file loaded)>");
            return "<TiffReader " + std::to_string(r.width()) + "x"
                   + std::to_string(r.height()) + "x"
    ...
    // HDF5Reader
    // =========================================================================
    py::class_<HDF5Reader>(m, "HDF5Reader",
        "Reads 3-D datasets from HDF5 files.")
        ...
        .def("read_file", &HDF5Reader::readFile,
             py::arg("filename"),
             py::arg("hdf5dataset"),
        ...
        .def("threshold",
             py::overload_cast<double, amrex::iMultiFab&>(
                 &HDF5Reader::threshold, py::const_),
             py::arg("raw_threshold"),
             py::arg("mf"))
        ...
        .def("get_attribute", &HDF5Reader::getAttribute,
             py::arg("attr_name"),
        ...
        .def("__repr__", [](const HDF5Reader& r) {
            if (!r.isRead()) return std::string("<HDF5Reader (no file loaded)>");
            return "<HDF5Reader " + std::to_string(r.width()) + "x"
                   + std::to_string(r.height()) + "x"
                   + std::to_string(r.depth()) + ">";
        });
    ...
    // RawReader
    // =========================================================================
    py::class_<RawReader>(m, "RawReader",
        "Reads 3-D voxel data from flat binary (RAW) files.")

        .def(py::init<>())

        .def(py::init<const std::string&, int, int, int, RawDataType>(),
             py::arg("filename"),
             py::arg("width"),
             py::arg("height"),
             py::arg("depth"),
        ...
        .def("read_file", &RawReader::readFile,
             py::arg("filename"),
             py::arg("width"),
             py::arg("height"),
             py::arg("depth"),
        ...
        .def("threshold",
             py::overload_cast<double, amrex::iMultiFab&>(
                 &RawReader::threshold, py::const_),
             py::arg("threshold_value"),
             py::arg("mf"))
        ...
        .def("get_value", &RawReader::getValue,
             py::arg("i"), py::arg("j"), py::arg("k"),
             "Retrieve a single voxel value (converted to float).")

        .def("__repr__", [](const RawReader& r) {
            if (!r.isRead()) return std::string("<RawReader (no file loaded)>");
            return "<RawReader " + std::to_string(r.width()) + "x"
                   + std::to_string(r.height()) + "x"
                   + std::to_string(r.depth()) + ">";
        });
    ...
    // DatReader
    // =========================================================================
    py::class_<DatReader>(m, "DatReader",
        "Reads 3-D image data from legacy DAT binary files.")
        ...
        .def("read_file", &DatReader::readFile,
             py::arg("filename"))

        .def("threshold",
             py::overload_cast<DatReader::DataType, amrex::iMultiFab&>(
        ...
        .def("get_raw_value", &DatReader::getRawValue,
             py::arg("i"), py::arg("j"), py::arg("k"))

        .def("__repr__", [](const DatReader& r) {
            if (!r.isRead()) return std::string("<DatReader (no file loaded)>");
            return "<DatReader " + std::to_string(r.width()) + "x"
                   + std::to_string(r.height()) + "x"
                   + std::to_string(r.depth()) + ">";
        });
}
