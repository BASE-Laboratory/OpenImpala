...
void runDryRunChecks(const amrex::Geometry& geom, const amrex::BoxArray& ba,
                     ...
                     const amrex::Box& domain_box, const std::string& filename,
                     int phase_id, int box_size, amrex::Real threshold_val,
                     const std::string& calc_method, int verbose) {
...
void runFlowThrough(const amrex::Geometry& geom, const amrex::BoxArray& ba,
                    ...
                    const std::string& provenance_sample_id,
                    const std::string& provenance_uri,
    ...
    if (directions_to_run.empty()) {
        amrex::Warning(
            "No valid directions specified in 'direction' input. Skipping tortuosity.");
        return;
    }
    ...
    for (const auto& dir : directions_to_run) {
        ...
        OpenImpala::TortuosityHypre tort_solver(geom_tort, ba, dm, mf_phase, volume_fraction,
                                                ...
                                                results_path.string(), vlo, vhi, verbose,
                                                write_plotfiles);
...
int main(int argc, char* argv[]) {
    ...
    amrex::Initialize(argc, argv);
    {
        ...
        // ===================================================================
        if (dry_run) {
            runDryRunChecks(img.geom, img.ba, img.dm, img.mf_phase, img.domain_box,
                            main_filename, main_phase_id, main_box_size, main_threshold_val,
                            main_calc_method, main_verbose);
        ...
        if (!dry_run && (!rev_do_study || main_calc_method != "skip_if_rev")) {
            ...
            if (main_calc_method == "homogenization") {
                ...
                runFlowThrough(img.geom, img.ba, img.dm, img.mf_phase, img.domain_box,
                               ...
                               main_filename, provenance_sample_id, provenance_uri,
                               bpx_electrode, main_verbose, (main_write_plotfile != 0));
            }
        }
        ...
        amrex::ParallelDescriptor::ReduceRealMax(
            master_stop_time, amrex::ParallelDescriptor::IOProcessorNumber());
        if (amrex::ParallelDescriptor::IOProcessor()) {
